...
async function runSystemChecks(): Promise<CheckResult[]> {
  ...
  checks.push({
    ...
    message:
      invalidPosts > 0
        ? `${invalidPosts} posts have missing titles, slugs, or content.`
  ...
  const invalidProjects = allProjects.filter(
    (p) => !p.title || !p.description || !iconNames.includes(p.icon as any)
  );
  checks.push({
    name: 'Project Data',
    ...
    message:
      invalidProjects.length > 0
        ? `${invalidProjects.length} projects have missing fields or invalid icons.`
  ...
  checks.push({
    ...
    message:
      invalidTestimonials > 0
        ? `${invalidTestimonials} testimonials have missing authors or quotes.`
  ...
  const missingSettings = requiredSettings.filter(
    (key) => !settings.some((s) => s.key === key && s.value)
  );
  checks.push({
    name: 'Core Settings',
    ...
    message:
      missingSettings.length > 0
        ? `Missing values for: ${missingSettings.join(', ')}.`
        : 'All core settings are present.',
    category: 'Data Integrity',
  });
...
export default async function TroubleshootingPage() {
  ...
  const dataChecks = systemChecks.filter(
    (c) => c.category === 'Data Integrity'
  );
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center justify-between gap-4 rounded-lg border bg-background p-4">
            <p className="text-lg font-medium">Overall System Status</p>
          ...
          <Card>
            ...
            <CardContent>
              <p className="rounded-lg border-2 border-dashed p-4 text-center text-sm text-muted-foreground">
                Authentication is not yet implemented. Session and user data
                checks will be available here once Firebase Authentication is
                integrated.
              </p>
            </CardContent>
          </Card>
