...
import { ProjectSchema, type ProjectInputSchemaType, type IconName } from '@/lib/schemas';
...
export async function updateProject(id: number, data: ProjectInputSchemaType): Promise<Project> {
    const validatedData = ProjectSchema.omit({ id: true, createdAt: true, updatedAt: true }).parse(data);
    const { keyFeatures, ...rest } = validatedData;
